...
def sample_unconditional(model, batch_size, steps=256, temperature=None, top_k=None, top_p=None, callback=None,
    ...
    #########################################################
    #############################################################
    # Create a new tensor of shape (10,) to update the second column of the tensor
    new_column = torch.ones(8)
    #Update the second column of the tensor with the new column tensor
    for i in range(int(index_sample.shape[1] / 2)):
        index_sample[:, 2 * i] = index_sample[:, 2 * i + 1]


    for i in range(int(index_sample.shape[1] / 4)):
        index_sample[:, 4*i] = index_sample[:, 4*i+1]
        index_sample[:, 4*i+1] = index_sample[:, 4*i+2]
        index_sample[:, 4*i+2] = index_sample[:, 4*i+3]
...
def save_from_logs(logs, logdir, base_count, key="samples", cond_key=None):
    ...
    ########################################
    # print(xx.shape)
    x = logs["samples"].permute(1, 2, 0, 3).contiguous().view(3, 256, 2048)
    x = chw_to_pillow(x)
    x.save(os.path.join(logdir, f"{base_count:06}.png"))
