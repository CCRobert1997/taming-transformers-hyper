...
class WrappedNormal(torch.distributions.Distribution):
    ...
    def __init__(self, loc, scale, manifold, validate_args=None, softplus=False):
        ...
        #print(self.device)
        self.manifold_zero = torch.zeros(1, manifold.ndim).to(self.device)
        #self.manifold_zero = self.manifold.zero
        #self.manifold_zero = self.manifold_zero.to(device=torch.device(self.device))
        #self.manifold.zero = self.manifold_zero
    ...
    def rsample(self, sample_shape=torch.Size()):
        ...
        #print(self.manifold.zero.is_cuda)
        #manifold_zero = self.manifold.zero.cuda()
        #print(self.manifold.zero.is_cuda)

        #v = v / self.manifold.lambda_x(self.manifold.zero, keepdim=True)
        #u = self.manifold.transp(self.manifold.zero, self.loc, v)

        v = v / self.manifold.lambda_x(self.manifold_zero, keepdim=True)
        u = self.manifold.transp(self.manifold_zero, self.loc, v)
        #print(u.device)
        #print(self.loc.device)
        #print(self.manifold.c.device)
        #z = self.manifold.expmap(self.loc, u)
        z = expmap(self.loc, u, c=self.manifold.c)
        return z
    ...
    def log_prob(self, x):
        ...
        #print(loc.is_cuda)
        #print(x.is_cuda)
        #print(self.manifold.k.is_cuda)
        v = self.manifold.logmap(loc, x)

        v = self.manifold.transp(loc, self.manifold_zero, v)
        u = v * self.manifold.lambda_x(self.manifold_zero, keepdim=True)
